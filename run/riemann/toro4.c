                  ...
                  const void *context)
{
    ...
    EulerPrimitive left = {.density = 5.99924, .velocity = {.x = 19.5975}, .pressure = 460.894};
    EulerPrimitive right = {.density = 5.99242, .velocity = {.x = -6.19633}, .pressure = 46.0950};
    double gamma = property[EULER_HEAT_CAPACITY_RATIO];
    double loc = (center.x - 0.4) / time;
...
int main(int argc, char **argv)
{
    ...
    simulation_set_max_time(sim, 0.035);
