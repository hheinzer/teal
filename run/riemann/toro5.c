                  ...
                  const void *context)
{
    ...
    EulerPrimitive left = {.density = 1, .velocity = {.x = -19.59745}, .pressure = 1000};
    EulerPrimitive right = {.density = 1, .velocity = {.x = -19.59745}, .pressure = 0.01};
    double gamma = property[EULER_HEAT_CAPACITY_RATIO];
    double loc = (center.x - 0.8) / time;
...
int main(int argc, char **argv)
{
    ...
    simulation_set_max_time(sim, 0.012);
