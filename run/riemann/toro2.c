                  ...
                  const void *context)
{
    ...
    EulerPrimitive left = {.density = 1, .velocity = {.x = -2}, .pressure = 0.4};
    EulerPrimitive right = {.density = 1, .velocity = {.x = 2}, .pressure = 0.4};
    double gamma = property[EULER_HEAT_CAPACITY_RATIO];
    double loc = (center.x - 0.5) / time;
...
int main(int argc, char **argv)
{
    ...
    simulation_set_max_time(sim, 0.15);
