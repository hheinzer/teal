...
for fname in args.fnames:
    ...
    resolution = args.resolution if args.resolution is not None else mesh.n_cells
    ...
    for scalar in sample.array_names:
        if args.scalar in scalar:
            ...
            ax.plot(sample.points[:, 0], sample[scalar], label=label, zorder=zorder)
...
prefix = prefix if prefix[-1] != "_" else prefix[:-1]
fig.savefig(prefix + "_line.pdf", bbox_inches="tight")
