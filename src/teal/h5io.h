...
#define h5_dims(...) ((hsize_t[]){__VA_ARGS__})

#define h5_rank(dims) (sizeof(dims) / sizeof(*dims))
...
#define h5_dataset_write(name, buf, dims, loc) \
    x__h5_dataset_write_type(name, buf, dims, h5_rank(dims), h5_type(buf), loc)
