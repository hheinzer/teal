...
static void starpu(const double *g, const double dl, const double ul, const double pl,
                   ...
                   const double cr, double *pm, double *um);
...
void riemann(const double gamma, const double dl, const double ul, const double pl, const double dr,
    ...
    assert(g[4] * (cl + cr) > (ur - ul) && "vacuum generated");

    double pm, um;
    starpu(g, dl, ul, pl, dr, ur, pr, cl, cr, &pm, &um);
    sample(g, dl, ul, pl, dr, ur, pr, cl, cr, pm, um, s, d, u, p);
}
...
static void starpu(const double *g, const double dl, const double ul, const double pl,
    ...
    for (; iter < NRITER; ++iter) {
        prefun(g, pold, dl, pl, cl, &fl, &fld);
        prefun(g, pold, dr, pr, cr, &fr, &frd);
        *p = pold - (fl + fr + udiff) / (fld + frd);
        if (2 * fabs((*p - pold) / (*p + pold)) < EPS) break;
        pold = *p = MAX(EPS, *p);
    }
    assert(iter < NRITER && "Newton-Raphson divergence");
    *u = 0.5 * (ul + ur + fr - fl);
}
