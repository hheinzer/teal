...
double simulation2_run(Simulation *sim)
{
    assert(sim);
    ...
    if (prefix[0]) {
        mesh2_write(eqns->mesh, prefix);
        equations2_write(eqns, prefix, time, index++);
    }
    out_time += time;

    double *residual = teal2_calloc(stride, (int)sizeof(*residual));

    teal2_print("Running simulation");
    teal2_print("\t %13s %13s %13s %13s %13s", "iter", "time", "step", "residual", "wtime");
    ...
    while (iter < max_iter && time < max_time && !has_converged && !sig_terminate) {
        ...
        if (has_reached_time || has_reached_iter || has_converged || sig_terminate) {
            ...
            teal2_print("\t %13d %13g %13g %13g %13g", iter, time, step0, max_residual,
    ...
    teal2_print("\t computation time  : %g", wtime_end - wtime_beg);
