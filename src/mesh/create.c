...
static void compute_dims(tuple num_cells, number ndims, int *dims)
{
    number cells[3] = {lmax(1, num_cells.x), lmax(1, num_cells.y), lmax(1, num_cells.z)};
    dims[0] = dims[1] = dims[2] = 1;

    // sort axes by descending global cell count (try larger cuts first)
    number perm[3] = {0, 1, 2};
    if (cells[perm[1]] > cells[perm[0]]) {
        swap_number(&perm[0], &perm[1]);
    }
    if (cells[perm[2]] > cells[perm[1]]) {
        swap_number(&perm[1], &perm[2]);
    }
    if (cells[perm[1]] > cells[perm[0]]) {
        swap_number(&perm[0], &perm[1]);
    }

    number prime = sync.size;
    while (prime > 1) {
        number factor = 2;
...
static void split_bounds(scalar *min_coord, scalar *max_coord, number *num_cells, number dim,
                         number ndims, const int *dims, const int *coords)
{
    if (ndims > dim) {
        assert(!isclose(*min_coord, *max_coord));
        scalar width = (*max_coord - *min_coord) / *num_cells;
        number base = *num_cells / dims[dim];
        number extra = *num_cells % dims[dim];
        number offset = (coords[dim] * base) + lmin(coords[dim], extra);
                         ...
                         tuple num_nodes, const int *coords)
{
    ...
    number num = 0;
    number num_inner = 0;
    for (number k = 0; k < num_nodes.z; k++) {
        for (number j = 0; j < num_nodes.y; j++) {
            for (number i = 0; i < num_nodes.x; i++) {
...
static void compute_globals(MeshNodes *nodes, tuple num_nodes, const int *dims, const int *coords,
                            const int *neighbor)
{
    number *global = arena_malloc(nodes->num, sizeof(*global));
    number off_inner = sync_lexsum(nodes->num_inner);
    int tag_x = sync_tag();
    int tag_y = sync_tag();
    int tag_z = sync_tag();
    number num = 0;
    ...
    for (number k = 0; k < num_nodes.z; k++) {
        for (number j = 0; j < num_nodes.y; j++) {
            for (number i = 0; i < num_nodes.x; i++) {
                ...
                if (i == 0 && coords[0]) {
                    MPI_Recv(&global[num], 1, MPI_NUMBER, neighbor[0], tag_x, sync.comm,
                             MPI_STATUS_IGNORE);
                }
                if (j == 0 && coords[1]) {
                    MPI_Recv(&global[num], 1, MPI_NUMBER, neighbor[2], tag_y, sync.comm,
                             MPI_STATUS_IGNORE);
                }
                if (k == 0 && coords[2]) {
                    MPI_Recv(&global[num], 1, MPI_NUMBER, neighbor[4], tag_z, sync.comm,
                             MPI_STATUS_IGNORE);
                }
                if (i == num_nodes.x - 1 && coords[0] < dims[0] - 1) {
                    MPI_Send(&global[num], 1, MPI_NUMBER, neighbor[1], tag_x, sync.comm);
                }
                if (j == num_nodes.y - 1 && coords[1] < dims[1] - 1) {
                    MPI_Send(&global[num], 1, MPI_NUMBER, neighbor[3], tag_y, sync.comm);
                }
                if (k == num_nodes.z - 1 && coords[2] < dims[2] - 1) {
                    MPI_Send(&global[num], 1, MPI_NUMBER, neighbor[5], tag_z, sync.comm);
                }
                num += 1;
            }
        }
...
static bool is_edge_side(const int *dims, const int *coords, number idx)
{
...
static void create_cells(MeshCells *cells, tuple num_cells, tuple num_nodes, number ndims,
                         const int *dims, const int *coords, const int *neighbor)
{
    ...
    number *idx = arena_malloc(cells->num * MAX_CELL_NODES, sizeof(*idx));

    off[0] = 0;

    number num = 0;
    for (number k = 0; k < num_cells.z; k++) {
        for (number j = 0; j < num_cells.y; j++) {
            for (number i = 0; i < num_cells.x; i++) {
                off[num + 1] = off[num];
    ...
    if (ndims >= 1) {
        for (number i = 0; i <= num_cells.x; i += num_cells.x) {
            for (number j = 0; j < num_cells.y; j++) {
                for (number k = 0; k < num_cells.z; k++) {
                    off[num + 1] = off[num];
                    idx[off[num + 1]++] =
    ...
    if (ndims >= 2) {
        for (number j = 0; j <= num_cells.y; j += num_cells.y) {
            for (number k = 0; k < num_cells.z; k++) {
                for (number i = 0; i < num_cells.x; i++) {
                    off[num + 1] = off[num];
                    idx[off[num + 1]++] =
    ...
    if (ndims >= 3) {
        for (number k = 0; k <= num_cells.z; k += num_cells.z) {
            for (number i = 0; i < num_cells.x; i++) {
                for (number j = 0; j < num_cells.y; j++) {
                    off[num + 1] = off[num];
                    idx[off[num + 1]++] =
...
static void compute_node_map(const MeshNodes *nodes, tuple num_nodes, const int *coords,
                             number *map)
{
    number num = 0;
    number num_inner = 0;
    number num_neighbors = 0;
    for (number k = 0; k < num_nodes.z; k++) {
        for (number j = 0; j < num_nodes.y; j++) {
            for (number i = 0; i < num_nodes.x; i++) {
                if (!(i == 0 && coords[0]) && !(j == 0 && coords[1]) && !(k == 0 && coords[2])) {
...
static void compute_cell_map(const MeshCells *cells, tuple num_cells, number ndims, const int *dims,
                             const int *coords, const int *neighbor, number *map)
{
    number num = 0;
    number num_inner = 0;
    number num_ghost = 0;
    ...
    for (number i = 0; i < 2 * ndims; i++) {
        for (number j = 0; j < num_cells_side(num_cells, i); j++) {
            if (neighbor[i] == MPI_PROC_NULL) {
...
static void reorder(MeshNodes *nodes, MeshCells *cells, tuple num_cells, tuple num_nodes,
                    number ndims, const int *dims, const int *coords, const int *neighbor)
{
    Arena save = arena_save();

    number num = lmax(nodes->num, cells->num);
    number *map = arena_malloc(num, sizeof(*map));
...
static void create_entities(MeshEntities *entities, tuple num_cells, vector del_coord, number ndims,
...
Mesh *mesh_create(vector min_coord, vector max_coord, tuple num_cells, flags periodic)
{
    ...
    int dims[3] = {0};
    number ndims = (num_cells.x > 1) + (num_cells.y > 1) + (num_cells.z > 1);
