...
static int compute_dims(int *dims, Triple num_cells)
{
    ...
    if (sync.size > count[0] * count[1] * count[2]) {
        teal_error("using more rank (%d) than cells (%ld) is not supported", sync.size,
...
static long *compute_globals(Mesh *mesh, Triple num_cells, const int *dims, const int *coords,
                             const int *neighbor)
{
    long *global = teal_calloc(mesh->nodes.num, sizeof(*global));
    ...
    for (int k = 0; k < num_cells.z + 1; k++) {
        for (int j = 0; j < num_cells.y + 1; j++) {
            for (int i = 0; i < num_cells.x + 1; i++) {
                ...
                if (i == 0 && coords[0]) {
                    MPI_Recv(&global[num], 1, MPI_LONG, neighbor[0], 0, sync.comm,
                             MPI_STATUS_IGNORE);
                }
                if (j == 0 && coords[1]) {
                    MPI_Recv(&global[num], 1, MPI_LONG, neighbor[2], 1, sync.comm,
                             MPI_STATUS_IGNORE);
                }
                if (k == 0 && coords[2]) {
                    MPI_Recv(&global[num], 1, MPI_LONG, neighbor[4], 2, sync.comm,
                             MPI_STATUS_IGNORE);
                }
                if (i == num_cells.x && coords[0] < dims[0] - 1) {
                    MPI_Send(&global[num], 1, MPI_LONG, neighbor[1], 0, sync.comm);
                }
                if (j == num_cells.y && coords[1] < dims[1] - 1) {
                    MPI_Send(&global[num], 1, MPI_LONG, neighbor[3], 1, sync.comm);
                }
                if (k == num_cells.z && coords[2] < dims[2] - 1) {
                    MPI_Send(&global[num], 1, MPI_LONG, neighbor[5], 2, sync.comm);
                }
                num += 1;
            }
        }
...
static int num_cells_side(Triple num_cells, int idx)
{
    switch (idx / 2) {
        ...
        default: teal_error("invalid index (%d)", idx);
    }
}
                         ...
                         const int *neighbor, int ndims)
{
    ...
    int *node_off = teal_calloc(mesh->cells.num + 1, sizeof(*node_off));
    int *node_idx = teal_calloc(mesh->cells.num * MAX_CELL_NODES, sizeof(*node_idx));
    ...
    mesh->cells.node.idx = teal_realloc(node_idx, node_off[mesh->cells.num], sizeof(*node_idx));
}
                            ...
                            const int *periods, const int *coords, const int *neighbor, int ndims)
{
    ...
    String *name = teal_calloc(num_entities, sizeof(*name));
    int *cell_off = teal_calloc(num_entities + 1, sizeof(*cell_off));
                             ...
                             const int *neighbor, int ndims)
{
    ...
    int *send_idx = teal_calloc(num_indices, sizeof(*send_idx));
...
static void reorder_cells(Mesh *mesh, Triple num_cells, const int *dims, const int *coords,
                          const int *neighbor, int ndims)
{
    int *map = teal_calloc(mesh->cells.num, sizeof(*map));
    ...
    mesh_reorder_cells(mesh, map, 0, mesh->cells.num);

    teal_free(map);
}

Mesh *mesh_create(Vector min_coord, Vector max_coord, Triple num_cells, Triple periodic)
{
    int dims[3];
    ...
    for (int i = 0; i < ndims; i++) {
        MPI_Cart_shift(sync.comm, i, 1, &neighbor[(2 * i) + 0], &neighbor[(2 * i) + 1]);
    }
