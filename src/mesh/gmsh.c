...
static void read_physicals(Gmsh *gmsh, MPI_File file)
{
    ...
    Physical *physical = teal_calloc(gmsh->physicals.num, sizeof(*physical));
    for (int32_t i = 0; i < gmsh->physicals.num; i++) {
        ...
        parse_string(file, physical[i].name, sizeof(physical[i].name));
    }
...
static void read_points(Gmsh *gmsh, MPI_File file, int mode)
{
    ...
    Point *point = teal_calloc((int)gmsh->entities.num_points, sizeof(*point));
    for (uint64_t i = 0; i < gmsh->entities.num_points; i++) {
        ...
        int32_t *physical_tag = teal_calloc(num_physical_tags, sizeof(*physical_tag));
...
static void read_curves(Gmsh *gmsh, MPI_File file, int mode)
{
    ...
    Curve *curve = teal_calloc((int)gmsh->entities.num_curves, sizeof(*curve));
    for (uint64_t i = 0; i < gmsh->entities.num_curves; i++) {
        ...
        int32_t *point_tag = teal_calloc(num_point_tags, sizeof(*point_tag));
...
static void read_surfaces(Gmsh *gmsh, MPI_File file, int mode)
{
    ...
    Surface *surface = teal_calloc((int)gmsh->entities.num_surfaces, sizeof(*surface));
    for (uint64_t i = 0; i < gmsh->entities.num_surfaces; i++) {
        ...
        int32_t *curve_tag = teal_calloc(num_curve_tags, sizeof(*curve_tag));
...
static void read_volumes(Gmsh *gmsh, MPI_File file, int mode)
{
    ...
    Volume *volume = teal_calloc((int)gmsh->entities.num_volumes, sizeof(*volume));
    for (uint64_t i = 0; i < gmsh->entities.num_volumes; i++) {
        ...
        int32_t *surface_tag = teal_calloc(num_surface_tags, sizeof(*surface_tag));
...
static Slice slice_rank(uint64_t total)
{
    ...
    uint64_t rank = (uint64_t)sync.rank;
    uint64_t size = (uint64_t)sync.size;
...
static void read_nodes(Gmsh *gmsh, MPI_File file, int mode)
{
    ...
    NodeBlock *block = teal_calloc((int)gmsh->nodes.num_blocks, sizeof(*block));
    uint64_t offset = 0;
    for (uint64_t i = 0; i < gmsh->nodes.num_blocks; i++) {
        parse(file, &block[i].entity_dim, 1, MPI_INT32_T, mode);
        parse(file, &block[i].entity_tag, 1, MPI_INT32_T, mode);
        parse(file, &block[i].parametric, 1, MPI_INT32_T, mode);
        parse(file, &block[i].num_nodes, 1, MPI_UINT64_T, mode);

        uint64_t tot = block[i].num_nodes;
        int num = num_overlap(slice.beg, slice.end, offset, tot);
        block[i].num_nodes = num;

        block[i].tag = teal_calloc(num, sizeof(*block[i].tag));
        parse(file, block[i].tag, num, MPI_UINT64_T, mode | SPLIT);

        int len = len_coord(&block[i]);
        block[i].coord = teal_calloc(num * len, sizeof(*block[i].coord));
        parse(file, block[i].coord, num * len, MPI_DOUBLE, mode | SPLIT);
...
static void read_elements(Gmsh *gmsh, MPI_File file, int mode)
{
    ...
    ElementBlock *block = teal_calloc((int)gmsh->elements.num_blocks, sizeof(*block));
    uint64_t offset = 0;
    for (uint64_t i = 0; i < gmsh->elements.num_blocks; i++) {
        parse(file, &block[i].entity_dim, 1, MPI_INT32_T, mode);
        parse(file, &block[i].entity_tag, 1, MPI_INT32_T, mode);
        parse(file, &block[i].element_type, 1, MPI_INT32_T, mode);
        parse(file, &block[i].num_elements, 1, MPI_UINT64_T, mode);
...
static void read_periodic(Gmsh *gmsh, MPI_File file, int mode)
{
    ...
    Link *link = teal_calloc((int)gmsh->periodic.num_links, sizeof(*link));
    for (uint64_t i = 0; i < gmsh->periodic.num_links; i++) {
        parse(file, &link[i].entity_dim, 1, MPI_INT32_T, mode);
        parse(file, &link[i].entity_tag, 1, MPI_INT32_T, mode);
        parse(file, &link[i].entity_tag_master, 1, MPI_INT32_T, mode);
        parse(file, &link[i].num_affine, 1, MPI_UINT64_T, mode);

        assert(link[i].num_affine <= INT_MAX);
        link[i].affine = teal_calloc((int)link[i].num_affine, sizeof(*link[i].affine));
        parse(file, link[i].affine, (int)link[i].num_affine, MPI_DOUBLE, mode);

        parse(file, &link[i].num_nodes, 1, MPI_UINT64_T, mode);
        uint64_t total = link[i].num_nodes;
        uint64_t base = total / sync.size;
        uint64_t extra = total % sync.size;

        assert(base <= INT_MAX && sync.rank >= 0);
        int num = (int)base + ((uint64_t)sync.rank < extra);
        link[i].num_nodes = num;

        link[i].node_tag = teal_calloc(num, sizeof(*link[i].node_tag));
        link[i].node_tag_master = teal_calloc(num, sizeof(*link[i].node_tag_master));

        uint64_t (*buf)[2] = teal_calloc(num, sizeof(*buf));
        parse(file, buf, num * 2, MPI_UINT64_T, mode | SPLIT);
...
{
    MPI_File file = parse_open(fname);

    Gmsh *gmsh = teal_calloc(1, sizeof(*gmsh));
    int mode = 0;
    String section;
    while (parse_string(file, section, sizeof(section))) {
        if (!strcmp(section, "$MeshFormat")) {
            ...
            teal_error("unsupported mesh section (%s)", section);
        }
    }
                                  ...
                                  uint64_t *num_physical_tags)
{
    switch (block->entity_dim) {
        case 2:
            ...
            teal_error("could not find surface entity (%d)", block->entity_tag);
        case 3:
            for (uint64_t i = 0; i < gmsh->entities.num_volumes; i++) {
            ...
            teal_error("could not find volume entity (%d)", block->entity_tag);
        default: teal_error("unsupported element block dimension (%d)", block->entity_dim);
    }
}
...
static Physical *get_physical(const ElementBlock *block, const Gmsh *gmsh)
{
    ...
    if (num_physical_tags != 1) {
        teal_error("unsupported number of physical tags (%" PRIu64 ")", num_physical_tags);
    }
    for (int32_t i = 0; i < gmsh->physicals.num; i++) {
    ...
    teal_error("could not find physical name (%d, %d)", block->entity_dim, physical_tag[0]);
}
...
static Entity *extract(const Gmsh *gmsh)
{
    ...
    Entity *entity = teal_calloc(num_blocks, sizeof(*entity));
    for (uint64_t i = 0; i < gmsh->elements.num_blocks; i++) {
...
static void create_nodes(Grid *grid, const Gmsh *gmsh)
{
    if (gmsh->nodes.num_blocks == 0 || gmsh->nodes.num_nodes == 0) {
        teal_error("mesh does not contain any nodes");
    }
...
static void create_cells(Grid *grid, const Entity *entity, const Gmsh *gmsh)
{
    if (gmsh->elements.num_blocks == 0 || gmsh->elements.num_elements == 0) {
        teal_error("mesh does not contain any elements");
    }

    assert(gmsh->elements.num_elements <= INT_MAX);
    int num_cells = (int)gmsh->elements.num_elements;

    long *node_off = teal_calloc(num_cells + 1, sizeof(*node_off));
    long *node_tag = teal_calloc(num_cells * MAX_CELL_NODES, sizeof(*node_tag));
...
static int count_entities(Grid *grid, const Entity *entity, const Gmsh *gmsh)
{
    if (entity[0].physical->dim != 3) {
        teal_error("mesh does not contain any volumes");
    }
        ...
        if (!entity[i].link) {
            if (entity[i].physical != entity[i - 1].physical) {
                if (entity[i].physical->dim == 3) {
                    ...
                    teal_error("unsupported physical dimension (%d)", entity[i].physical->dim);
                }
            }
        }
        else {
            if (entity[i].link->entity_dim != 2) {
                teal_error("unsupported periodic dimension (%d)", entity[i].physical->dim);
            }
            num_periodic += 1;
...
static void get_affine_transformation(Matrix *rotation, Vector *translation, const Entity *entity)
{
    ...
    if (entity->block->entity_tag == entity->link->entity_tag_master) {
        ...
        *translation = vector_mul(-1, matrix_vector(*rotation, *translation));
    }
}
...
static void create_entities(Grid *grid, const Entity *entity, const Gmsh *gmsh)
{
    ...
    String *name = teal_calloc(grid->entities.num, sizeof(*name));
    int *cell_off = teal_calloc(grid->entities.num + 1, sizeof(*cell_off));
    int *periodic = teal_calloc(grid->entities.num, sizeof(*periodic));
    Matrix *rotation = teal_calloc(grid->entities.num, sizeof(*rotation));
    Vector *translation = teal_calloc(grid->entities.num, sizeof(*translation));
    long *node_off = teal_calloc(grid->entities.num + 1, sizeof(*node_off));
    long *node_tag = teal_calloc(num_nodes, sizeof(*node_tag));
...
Grid *grid_gmsh(const char *fname)
{
    ...
    Grid *grid = teal_calloc(1, sizeof(*grid));
