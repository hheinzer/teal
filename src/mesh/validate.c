...
{
    Kdtree *tree = kdtree_init(point, num);
    for (int i = 0; i < num; i++) {
        int idx[2];
        int len = kdtree_nearest(tree, point[i], idx, 2);
        ensure(len > 0);
        for (int j = 0; j < len; j++) {
...
static void validate_nodes(const Mesh *mesh)
{
    ...
    sync_prefix(&beg_global, 1, MPI_LONG);
    ...
    Vector *coord = teal_calloc(mesh->nodes.num, sizeof(*coord));
...
static void validate_neighbors(const Mesh *mesh)
{
    ...
    int *rank = teal_calloc(mesh->neighbors.num, sizeof(*rank));
    ...
    for (int i = 0; i < mesh->neighbors.num; i++) {
        ...
        ensure(mesh->neighbors.rank[i] < sync.size);
        sync_irecv(&req_recv[i], &rank[i], 1, mesh->neighbors.rank[i], mesh->neighbors.tag[i][0],
                   MPI_INT, 1);
        sync_isend(&req_send[i], &sync.rank, 1, mesh->neighbors.rank[i], mesh->neighbors.tag[i][1],
                   MPI_INT, 1);
    }
...
void mesh_validate(const Mesh *mesh)
{
    assert(mesh);
    validate_nodes(mesh);
    validate_cells(mesh);
