...
Grid *grid_init(const char *fname)
{
    ...
    teal_error("unsupported file extension (%s)", ext);
}
...
void grid_deinit(Grid *grid)
{
    ...
    teal_free(grid->entities.name);
    teal_free(grid->entities.cell_off);
    teal_free(grid->entities.periodic);
    teal_free(grid->entities.rotation);
    teal_free(grid->entities.translation);
    teal_free(grid->entities.node_off);
    teal_free(grid->entities.node_idx);
...
long *grid_tag_to_idx(const Grid *grid, const long *node_tag, int num_tags)
{
    ...
    Map *map = teal_calloc(cap, sizeof(*map));
    ...
    int *converted = teal_calloc(num_tags, sizeof(*converted));
    long *node_idx = teal_calloc(num_tags, sizeof(*node_idx));
    ...
    for (int rank = 0; rank < sync.size; rank++) {
        ...
        sync_rotate(map, &num, cap, type, 1);
    }
    ...
    for (int i = 0; i < num_tags; i++) {
        if (!converted[i]) {
            teal_error("node tag (%ld) was not converted to index", node_tag[i]);
        }
    }
