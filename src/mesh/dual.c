...
{
    Dual *dual = teal_calloc(1, sizeof(*dual));

    dual->dist = teal_calloc(sync.size + 1, sizeof(*dual->dist));
    sync_offsets(&(long){grid->cells.off_periodic}, dual->dist, 1, MPI_LONG);
    ...
    int ret = ParMETIS_V3_Mesh2Dual(dual->dist, grid->cells.node_off, grid->cells.node_idx,
                                    &numflag, &ncommon, &xadj, &adjncy, &sync.comm);
    if (ret != METIS_OK) {
        teal_error("ParMETIS_V3_Mesh2Dual failed (%d)", ret);
    }

    dual->xadj = teal_calloc(grid->cells.off_periodic + 1, sizeof(*xadj));
    copy(dual->xadj, xadj, grid->cells.off_periodic + 1, sizeof(*xadj));

    assert(xadj[grid->cells.off_periodic] <= INT_MAX);
    dual->adjncy = teal_calloc((int)xadj[grid->cells.off_periodic], sizeof(*adjncy));
...
static void collect_outer(long *part, const Dual *dual, const Grid *grid)
{
    ...
    long *adjncy = teal_calloc(num_cells, sizeof(*adjncy));
    int num = 0;
    ...
    long *recv = teal_calloc(num_cells, sizeof(*recv));
    sync_collect(part, recv, adjncy, grid->cells.off_periodic, num_cells, MPI_LONG, 1);
...
{
    long *vwgt = teal_calloc(grid->cells.off_periodic, sizeof(*vwgt));
    ...
    long numflag = 0;
    long ncon = 1;
    long nparts = sync.size;

    assert(ncon <= INT_MAX / nparts);
    double *tpwgts = teal_calloc((int)(ncon * nparts), sizeof(*tpwgts));
    for (int i = 0; i < (int)(ncon * nparts); i++) {
        tpwgts[i] = 1 / (double)nparts;
    }

    double *ubvec = teal_calloc((int)ncon, sizeof(*ubvec));
    for (int i = 0; i < ncon; i++) {
        ubvec[i] = 1.05;
    }

    long *part = teal_calloc(grid->cells.off_periodic, sizeof(*part));
    ...
    if (ret != METIS_OK) {
        teal_error("ParMETIS_V3_PartKway failed (%d)", ret);
    }

    if (sync.rank == 0) {
        teal_verbose("ParMETIS_V3_PartKway edgecut = %ld", edgecut);
    }

    long *num_inner = teal_calloc(sync.size, sizeof(*num_inner));
    for (int i = 0; i < grid->cells.num_inner; i++) {
        num_inner[part[i]] += 1;
    }
    sync_sum(num_inner, sync.size, MPI_LONG);

    if (num_inner[sync.rank] <= 0 || INT_MAX < num_inner[sync.rank]) {
        teal_error("invalid partition size (%ld)", num_inner[sync.rank]);
    }
...
static MPI_Datatype datatype_map(void)
{
    ...
    return sync_resized(datatype, sizeof(Map));
}
...
static Map *compute_node_map(const Grid *grid, int *num_nodes)
{
    ...
    Map *map = teal_calloc((int)max_nodes, sizeof(*map));
    int num = 0;
    ...
    *num_nodes = unique(map, num, sizeof(*map), compare_map);
    ...
    Map *periodic = teal_calloc(cap, sizeof(*periodic));
    num = 0;
    for (int i = grid->entities.off_boundary; i < grid->entities.num; i++) {
    ...
    for (int rank = 0; rank < sync.size; rank++) {
        ...
        sync_rotate(periodic, &num, cap, type, 1);
    }
    assert(num == num_periodic);
    MPI_Type_free(&type);
    ...
    for (int i = 0; i < *num_nodes; i++) {
        if (map[i].peer == -1) {
            teal_error("missing periodic peer node (%d, %" PRIi64 ")", map[i].entity, map[i].node);
        }
    }
...
static long *collect_edges(const Dual *dual, const Grid *grid, int num_edges)
{
    ...
    Cell *cell = teal_calloc(num_edges, sizeof(*cell));
    int num = 0;
    for (int i = grid->entities.off_boundary; i < grid->entities.num; i++) {
        for (int j = grid->entities.cell_off[i]; j < grid->entities.cell_off[i + 1]; j++) {
            cell[num].entity = i;
    ...
    Cell *peer = teal_calloc(cap, sizeof(*peer));
    ...
    for (int rank = 0; rank < sync.size; rank++) {
        ...
        sync_rotate(peer, &num, cap, type, 1);
    }
    assert(num == num_edges);
    MPI_Type_free(&type);

    long *edge = teal_calloc(num_edges, sizeof(*edge));
    for (int i = 0; i < num_edges; i++) {
        if (peer[i].inner == -1) {
            teal_error("missing periodic peer cell (%d, %ld)", peer[i].entity, cell[i].inner);
        }
...
void dual_periodic(Dual *dual, const Grid *grid)
{
    ...
    long *xadj = teal_calloc(grid->cells.off_periodic + 1, sizeof(*xadj));
