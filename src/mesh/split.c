...
static int reorder_cells(Mesh *mesh, Vector root, Vector normal, int index)
{
    ...
    int *map = teal_calloc(num_cells, sizeof(*map));
    int num = 0;
    for (int i = beg; i < end; i++) {
        Vector mean = {0};
        for (int j = mesh->cells.node.off[i]; j < mesh->cells.node.off[i + 1]; j++) {
            vector_iadd(&mean, mesh->nodes.coord[mesh->cells.node.idx[j]]);
        }
        vector_idiv(&mean, mesh->cells.node.off[i + 1] - mesh->cells.node.off[i]);
        map[num++] = (vector_dot(vector_sub(mean, root), normal) <= 0);
    }
    ...
    mesh_reorder_cells(mesh, map, beg, end);
...
static void split_entity(Mesh *mesh, int index, int offset)
{
    ...
    String *name = teal_realloc(mesh->entities.name, num_entities, sizeof(*name));
    move(&name[index + 1], &name[index], num_move, sizeof(*name));
    strcat(name[index], "-a");
    strcat(name[index + 1], "-b");

    int *cell_off = teal_realloc(mesh->entities.cell_off, num_entities + 1, sizeof(*cell_off));
    move(&cell_off[index + 2], &cell_off[index + 1], num_move, sizeof(*cell_off));
    cell_off[index + 1] = cell_off[index] + offset;

    Matrix *rotation = teal_realloc(mesh->entities.rotation, num_entities, sizeof(*rotation));
    move(&rotation[index + 1], &rotation[index], num_move, sizeof(*rotation));

    Vector *translation =
        teal_realloc(mesh->entities.translation, num_entities, sizeof(*translation));
