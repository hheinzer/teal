                          ...
                          const MeshEntities *entities)
{
    ...
    idx_t ncommon = 3;
    Dual dual;
    long ret = ParMETIS_V3_Mesh2Dual(elmdist, eptr, eind, &numflag, &ncommon, &dual.xadj,
...
static void compute_partitioning(const MeshCells *cells, const Dual *dual, idx_t *part)
{
    ...
    if (option.num_refines > 0) {
        ...
        verbose("\t %4d %12ld %10d %s", 0, (long)edgecut, 0, "initial");
        ...
        for (long i = 0; i < option.num_refines; i++) {
            ...
            long delta = edgecut - last_edgecut;
            verbose("\t %4ld %12ld %+10ld %s", i + 1, (long)edgecut, delta, note);
...
static void resolve_comm_reorder(MeshNodes *nodes, Neighbor *neighbor, long *num_neighbors,
                                 long dst)
{
    Arena save = arena_save();

    long *rank = arena_malloc(sync.size, sizeof(*rank));
    MPI_Allgather(&dst, 1, MPI_LONG, rank, 1, MPI_LONG, sync.comm);
    ...
    long count = sync_lmax(*num_neighbors);
    tag = sync_tag();
...
static void redistribute_cells(MeshCells *cells, MeshEntities *entities, const idx_t *part)
{
    ...
    long *idx = realloc(cells->node.idx, cells->num * sizeof(*idx) * MAX_CELL_NODES);
    assert(idx);
...
static void append_neighbor_cells(MeshCells *cells, const Neighbor *neighbor, long num_neighbors)
{
    ...
    long *idx = realloc(cells->node.idx, num_cells * sizeof(*idx) * MAX_CELL_NODES);
    assert(idx);
                             ...
                             long tot, long *map)
{
    ...
    for (long i = 0; i < entities->num + 1; ++i) {
        for (long j = 0; j < sync.size; ++j) {
            off[i][j + 1] += off[i][j];
        }
        for (long j = 0; j < sync.size + 1; ++j) {
            off[i][j] += base;
        }
