...
static long *collect_adjncys(const Dual *dual, const long *part, const Grid *grid, int *num_adjncys)
{
    ...
    int *cur_send = teal_calloc(sync.size, sizeof(*cur_send));
    copy(cur_send, off_send, sync.size, sizeof(*cur_send));

    int tot_send = off_send[sync.size];
    long *send = teal_calloc(tot_send, sizeof(*send));
    for (int i = 0; i < grid->cells.off_periodic; i++) {
    ...
    long *recv = teal_calloc(tot_recv, sizeof(*recv));
...
{
    long num_send = dual->dist[sync.rank + 1] - dual->dist[sync.rank];
    assert(0 <= num_send && num_send <= INT_MAX);

    long *recv = teal_calloc(num_adjncys, sizeof(*recv));
    sync_collect(part, recv, adjncy, (int)num_send, num_adjncys, MPI_LONG, 1);
...
static void decompose_comm(const Dual *dual, const long *part_, const Grid *grid)
{
    ...
    int *count = teal_calloc(sync.size, sizeof(*count));
    ...
    int *rank = teal_calloc(degree, sizeof(*rank));
    int *weight = teal_calloc(degree, sizeof(*weight));
    ...
    MPI_Comm comm;
    int ret = MPI_Dist_graph_create_adjacent(sync.comm, degree, rank, weight, degree, rank, weight,
                                             MPI_INFO_NULL, reorder, &comm);
    if (ret != MPI_SUCCESS) {
        teal_error("MPI_Dist_graph_create_adjacent failed (%d)", ret);
    }
    sync_reinit(comm);
...
static MPI_Datatype datatype_cell(void)
{
    ...
    return sync_resized(datatype, sizeof(Cell));
}
...
static void redistribute_cells(Grid *grid, const long *part)
{
    ...
    Cell *send = teal_calloc(grid->cells.off_periodic, sizeof(*send));
    ...
    int tot_recv = off_recv[sync.size];
    Cell *recv = teal_calloc(tot_recv, sizeof(*recv));
    ...
    int *cell_off = teal_calloc(grid->entities.num + 1, sizeof(*cell_off));
    long *node_off = teal_calloc(tot_recv + 1, sizeof(*node_off));
    long *node_idx = teal_calloc(tot_recv * MAX_CELL_NODES, sizeof(*node_idx));
    ...
    teal_free(grid->cells.node_off);
    teal_free(grid->cells.node_idx);
    teal_free(grid->entities.cell_off);
    ...
    grid->cells.node_idx = teal_realloc(node_idx, num_indices, sizeof(*node_idx));
...
{
    int *rank = teal_calloc(sync.size, sizeof(*rank));
    sync_gather(&dst, rank, 1, MPI_INT, 1);
    ...
    int num_nodes;
    sync_exchange(&grid->nodes.num, &num_nodes, 1, 1, dst, src, MPI_INT, 1);

    Vector *coord = teal_calloc(num_nodes, sizeof(*coord));
    sync_exchange(grid->nodes.coord, coord, grid->nodes.num, num_nodes, dst, src, MPI_DOUBLE, 3);

    teal_free(grid->nodes.coord);
...
static void create_periodic_graph(Mesh *mesh, const Grid *grid, const Cell *recv, int num_periodic)
{
    ...
    int (*tag)[2] = teal_calloc(num_neighbors, sizeof(*tag));
    int *rank = teal_calloc(num_neighbors, sizeof(*rank));
    int *recv_off = teal_calloc(num_neighbors + 1, sizeof(*recv_off));
    int *send_off = teal_calloc(num_neighbors + 1, sizeof(*send_off));
    int *send_idx = teal_calloc(num_periodic, sizeof(*send_idx));
    ...
    int *recv_idx = teal_calloc(num_periodic, sizeof(*recv_idx));
    for (int i = 0; i < num_periodic; i++) {
        recv_idx[i] = recv[i].idx;
    }

    MPI_Request *req_recv = teal_calloc(num_neighbors, sizeof(*req_recv));
    MPI_Request *req_send = teal_calloc(num_neighbors, sizeof(*req_send));
    for (int i = 0; i < num_neighbors; i++) {
        int off = send_off[i];
        int count = recv_off[i + 1] - recv_off[i];
        sync_irecv(&req_recv[i], &send_idx[off], count, rank[i], tag[i][0], MPI_INT, 1);
        sync_isend(&req_send[i], &recv_idx[off], count, rank[i], tag[i][1], MPI_INT, 1);
    }
...
static void collect_periodic(Mesh *mesh, Grid *grid)
{
    ...
    Cell *recv = teal_calloc(num_periodic, sizeof(*recv));
    ...
    for (int i = grid->entities.off_boundary; i < grid->entities.num; i++) {
        for (int j = grid->entities.cell_off[i]; j < grid->entities.cell_off[i + 1]; j++) {
            ...
            int rank = digitize(&adjncy, dual->dist, sync.size, sizeof(*dual->dist), compare_long);
            assert(0 <= rank && rank < sync.size);
...
static void append_neighbor_cells(Grid *grid, const Cell *recv, int tot_recv)
{
    ...
    long *inner = teal_calloc(num_inner, sizeof(*inner));
    ...
    long *node_off = teal_realloc(grid->cells.node_off, num_cells + 1, sizeof(*node_off));
    long *node_idx = teal_realloc(grid->cells.node_idx, max_indices, sizeof(*node_idx));
                                  ...
                                  const int *off_send, const int *idx_send, const Grid *grid)
{
    ...
    int num_periodic = grid->cells.off_periodic - grid->cells.off_boundary;
    int num_indices = off_send[sync.size] + num_periodic;

    int (*tag)[2] = teal_realloc(mesh->neighbors.tag, num_neighbors, sizeof(*tag));
    int *rank = teal_realloc(mesh->neighbors.rank, num_neighbors, sizeof(*rank));
    int *recv_off = teal_realloc(mesh->neighbors.recv_off, num_neighbors + 1, sizeof(*recv_off));
    int *send_off = teal_realloc(mesh->neighbors.send.off, num_neighbors + 1, sizeof(*send_off));
    int *send_idx = teal_realloc(mesh->neighbors.send.idx, num_indices, sizeof(*send_idx));
...
static void create_neighbors(Mesh *mesh, Grid *grid)
{
    ...
    long *adjncy = teal_calloc(num_adjncys, sizeof(*adjncy));
    copy(adjncy, dual->adjncy, num_adjncys, sizeof(*adjncy));
    num_adjncys = unique(adjncy, num_adjncys, sizeof(*adjncy), compare_long);

    int *num_inner = teal_calloc(sync.size, sizeof(*num_inner));
    sync_gather(&grid->cells.num_inner, num_inner, 1, MPI_INT, 1);

    int num = 0;
    while (num < num_adjncys) {
        int rank = digitize(&adjncy[num], dual->dist, sync.size, sizeof(*dual->dist), compare_long);
        long beg_inner = dual->dist[rank];
        long end_inner = dual->dist[rank] + num_inner[rank];
        if (rank == sync.rank || adjncy[num] < beg_inner || end_inner <= adjncy[num]) {
    ...
    int *rank = teal_calloc(num_adjncys, sizeof(*rank));
    for (int i = 0; i < num_adjncys; i++) {
        rank[i] = digitize(&adjncy[i], dual->dist, sync.size, sizeof(*dual->dist), compare_long);
    ...
    int *idx_recv = teal_calloc(num_adjncys, sizeof(*idx_recv));
    for (int i = 0; i < num_adjncys; i++) {
    ...
    Cell *send = teal_calloc(tot_send, sizeof(*send));
...
static MPI_Datatype datatype_node(void)
{
    ...
    return sync_resized(datatype, sizeof(Node));
}
    ...
    for (int rank = 0; rank < sync.size; rank++) {
        for (int i = 0; i < num; i++) {
            ...
            if (search(&key, node_idx, num_nodes, sizeof(*node_idx), compare_long)) {
                if (sync.rank < node[i].rank) {
                    node[i].rank = sync.rank;
                }
            }
        }
        sync_rotate(node, &num, cap, type, 1);
    }
...
{
    long *tag = teal_calloc(num_nodes, sizeof(*tag));
    for (int i = 0; i < num_nodes; i++) {
        if (node[i].new == -1) {
            teal_error("global node index was not converted (%ld)", node[i].old);
        }
        tag[i] = node[i].new;
    }
    return tag;
...
{
    Vector *coord = teal_calloc(num_nodes, sizeof(*coord));
    sync_collect(grid->nodes.coord, coord, node_idx, grid->nodes.num, num_nodes, MPI_DOUBLE, 3);

    Vector *sorted = teal_calloc(num_nodes, sizeof(*grid->nodes.coord));
    for (int i = 0; i < num_nodes; i++) {
        Map key = {.old = node_idx[i]};
        Map *val = search(&key, map, num_nodes, sizeof(*map), compare_map);
        if (!val) {
            teal_error("node index not found (%ld)", node_idx[i]);
        }
...
static void remap_cells_node_graph(Grid *grid, const Map *map, int num_nodes)
{
    for (int i = 0; i < grid->cells.num; i++) {
        for (long j = grid->cells.node_off[i]; j < grid->cells.node_off[i + 1]; j++) {
            ...
            Map *val = search(&key, map, num_nodes, sizeof(*map), compare_map);
            if (!val) {
                teal_error("node index not found (%ld)", grid->cells.node_idx[j]);
            }
...
static void remap_entities_node_graph(Grid *grid, const Map *map, int num_nodes)
{
    ...
    long *node_idx = teal_calloc(cap, sizeof(*node_idx));
    for (int i = 0; i < num; i++) {
    ...
    for (int rank = 0; rank < sync.size; rank++) {
        ...
        sync_rotate(node_idx, &num, cap, MPI_LONG, 1);
    }
    assert(num == grid->entities.node_off[grid->entities.num]);

    for (int i = 0; i < num; i++) {
        if (node_idx[i] < 0) {
            teal_error("periodic node index was not converted (%ld)", node_idx[i]);
        }
    }
...
static int partition_nodes(Grid *grid)
{
    ...
    long *node_idx = teal_calloc(max_nodes, sizeof(*node_idx));
    int num = 0;
    for (int i = 0; i < grid->cells.num; i++) {
        for (long j = grid->cells.node_off[i]; j < grid->cells.node_off[i + 1]; j++) {
    ...
    Node *node = teal_calloc(cap, sizeof(*node));
    for (int i = 0; i < num_nodes; i++) {
        node[i].rank = sync.rank;
    ...
    Map *map = teal_calloc(num_nodes, sizeof(*map));
    num = 0;
    for (int i = 0; i < num_nodes; i++) {
        if (node[i].rank == -sync.rank) {
            node[i].new = prefix + i;
...
{
    long *global = teal_calloc(grid->nodes.num, sizeof(*global));
...
{
    int *node_off = teal_calloc(grid->cells.num + 1, sizeof(*node_off));
    for (int i = 0; i < grid->cells.num + 1; i++) {
        assert(grid->cells.node_off[i] <= INT_MAX);
        node_off[i] = (int)grid->cells.node_off[i];
    }

    int *node_idx = teal_calloc(node_off[grid->cells.num], sizeof(*node_idx));
...
Mesh *mesh_read(const char *fname)
{
    ...
    Mesh *mesh = teal_calloc(1, sizeof(*mesh));
