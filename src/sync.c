...
void sync_u_wait(Equations *eqns) {
    wait(eqns->mesh->size, eqns->sync.recv_u);
}

void sync_u_end(Equations *eqns) {
    wait(eqns->mesh->size, eqns->sync.send_u);
}

void sync_dudx_begin(Equations *eqns) {
    const int size = eqns->mesh->size;
    const ALIAS(i_recv, eqns->mesh->sync.i_recv);
    const long nu = eqns->vars.nu * N_DIMS;
...
void sync_dudx_wait(Equations *eqns) {
    wait(eqns->mesh->size, eqns->sync.recv_dudx);
}

void sync_dudx_end(Equations *eqns) {
    wait(eqns->mesh->size, eqns->sync.send_dudx);
}
