...
static int keycmp(const long *key_a, const long n_key_a, const long *key_b, const long n_key_b);
...
{
    assert(dict->n_items < dict->max_items);
    long dist = 1;
...
void dict_print(const Dict *dict)
{
    ...
    for (long i = 0; i < dict->n_items; ++i) {
        array_print(item[i].key, item[i].n_key, ": ");
        array_print(item[i].val, item[i].n_val, "\n");
    }
}

static int keycmp(const long *key_a, const long n_key_a, const long *key_b, const long n_key_b)
{
    if (n_key_a < n_key_b) return -1;
    if (n_key_a > n_key_b) return 1;
    for (long i = 0; i < n_key_a; ++i) {
