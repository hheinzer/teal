...
void simulation_summary(const Simulation *sim)
{
    ...
    if (sim->iter.max < INT_MAX) {
        println("\t max iter         : %d", sim->iter.max);
    }
    if (sim->iter.output < INT_MAX) {
        println("\t output iter      : %d", sim->iter.output);
    }
    ...
    if (!strcmp(sim->advance.name, "lserk")) {
        ...
        println("\t time order       : %d", ctx->time_order);
        println("\t number of stages : %d", ctx->num_stages);
    }
    if (!strcmp(sim->advance.name, "implicit euler")) {
        ...
        println("\t krylov dimension : %d", ctx->krylov_dimension);
    }
