...
static int component_index(char chr)
{
    switch (chr) {
        ...
        case 'y': return 1;
        case 'z': return 2;
        default: teal_error("invalid component index (%c)", chr);
    }
}
...
void simulation_set_termination(Simulation *sim, const char *condition, double threshold)
{
    ...
    teal_error("invalid termination condition (%s)", condition);
}

void simulation_set_advance(Simulation *sim, const char *name, double courant, const void *context_)
{
    ...
    if (!strcmp(name, "lserk")) {
        RungeKutta *context = teal_calloc(1, sizeof(*context));
        if (context_) {
        ...
        if (!(1 <= context->time_order && context->time_order <= 3)) {
            teal_error("invalid time order (%d)", context->time_order);
        }
        if (!(1 <= context->num_stages && context->num_stages <= 6)) {
            teal_error("invalid number of stages (%d)", context->num_stages);
        }
    ...
    if (!strcmp(name, "implicit euler")) {
        NewtonKrylov *context = teal_calloc(1, sizeof(*context));
        if (context_) {
            memcpy(context, context_, sizeof(*context));
        }
        else {
        ...
        if (context->newton_tolerance <= 0) {
            teal_error("invalid newton tolerance (%g)", context->newton_tolerance);
        }
        if (context->krylov_tolerance <= 0) {
            teal_error("invalid krylov tolerance (%g)", context->krylov_tolerance);
        }
        if (context->krylov_dimension <= 0) {
            teal_error("invalid krylov dimension (%d)", context->krylov_dimension);
        }
        sim->advance.context = context;
        sim->advance.method = implicit_euler;
        return;
    }
    teal_error("invalid advance method (%s)", name);
}
