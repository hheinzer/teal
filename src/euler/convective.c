                    ...
                    Matrix basis)
{
    ...
    EulerPrimitive face_p = euler_riemann(left_p, right_p, gamma, 0);
...
static Vector roe_velocity(double weight, double sqrt_density_l, double sqrt_density_r, Euler left,
                           Euler right)
{
    return vector_mul(weight, vector_add(vector_mul(sqrt_density_l, left.velocity),
                ...
                Matrix basis)
{
    ...
    double velocity2 = vector_norm2(velocity);
                         ...
                         double signal_speed_r)
{
    ...
    flux->momentum =
        vector_mul(factor, vector_add(vector_sub(flux_l.momentum, flux_r.momentum), jump.momentum));
                         ...
                         double factor_k, double signal_speed)
{
    ...
    *flux = physical_flux(state_k);
    ...
    vector_iadd(&flux->momentum,
                vector_mul(signal_speed_k, vector_sub(star.momentum, state_k.momentum)));
                ...
                Matrix basis)
{
    ...
    vector_imul(&jump.momentum, signal_speed);
    ...
    flux->momentum =
        vector_div(vector_sub(vector_add(flux_l.momentum, flux_r.momentum), jump.momentum), 2);
                    ...
                    Matrix basis)
{
    ...
    flux->momentum = vector_div(vector_add(flux_l.momentum, flux_r.momentum), 2);
...
Convective *euler_convective(const char *name)
{
    ...
    teal_error("invalid convective flux (%s)", name);
}
