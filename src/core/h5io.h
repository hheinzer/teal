...
#define h5_dataset_write(name, buf, count, loc) \
    x__h5_dataset_write_type(name, buf, count, H5RANK(count), H5TYPE(buf), loc)

void x__h5_dataset_read_type(const char *name, void *buf, const hsize_t *count, int rank,
                             hid_t loc);
#define h5_dataset_read(name, buf, count, loc) \
    x__h5_dataset_read_type(name, buf, count, H5RANK(count), loc)
