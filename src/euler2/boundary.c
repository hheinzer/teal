                     ...
                     const void *context_)
{
    ...
    double velocity = vector2_dot(inner->velocity, basis.x);

    outer->density = inner->density;
    outer->velocity = vector2_sub(inner->velocity, vector2_mul(2 * velocity, basis.x));
                            ...
                            const void *context_)
{
    ...
    double velocity = vector2_dot(vector2_sub(context->velocity, inner->velocity), basis.x);
    ...
    outer->velocity = vector2_sub(context->velocity, vector2_mul(factor2, basis.x));
}
                             ...
                             const void *context_)
{
    ...
    outer->velocity = vector2_sub(inner->velocity, vector2_mul(factor, basis.x));
}
...
static void local_to_global(EulerPrimitive *global, Euler local, const double *property,
                            Matrix basis)
{
    local.velocity = vector2_div(local.momentum, local.density);
    local.pressure = (property[EULER_HEAT_CAPACITY_RATIO] - 1) *
                     (local.energy - (vector2_dot(local.momentum, local.velocity) / 2));
                     ...
                     const void *context_)
{
    ...
    double velocity2_i = vector2_norm2(inner.velocity);
...
Boundary *euler2_boundary(const char *name)
{
    ...
    teal2_error("invalid boundary condition (%s)", name);
}
