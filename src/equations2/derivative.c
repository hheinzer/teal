...
static void integrate_convective_flux_O1(const Equations *eqns, void *primitive_, void *derivative_)
{
    ...
    Exchange exchange = equations2_exchange(eqns, primitive, stride_p);
                    ...
                    VectorNorm correction, int stride)
{
    for (int i = 0; i < stride; i++) {
        double projection = vector2_dot(gradient_m[i], correction.unit);
        double adjust = projection - ((primitive_r[i] - primitive_l[i]) / correction.norm);
        vector2_isub(&gradient_m[i], vector2_mul(adjust, correction.unit));
    }
}
                                      ...
                                      void *derivative_, void *gradient_)
{
    ...
    Exchange exchange = equations2_exchange(eqns, gradient, 3 * stride_p);
...
{
    for (int i = 0; i < stride; i++) {
        primitive_k[i] = primitive[i] + vector2_dot(gradient[i], offset);
    }
}
...
void equations2_derivative(const Equations *eqns, void *primitive, void *derivative, double time)
{
    ...
    switch (eqns->space_order) {
        ...
        case 2: {
            ...
            Vector(*gradient)[stride] = teal2_calloc(num_cells, (int)sizeof(*gradient));
            ...
            if (eqns->limiter.kind != NONE) {
                equations2_limiter(eqns, primitive, gradient);
            }
        ...
        default: teal2_error("invalid space order (%d)", eqns->space_order);
    }
