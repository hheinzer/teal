...
static void link_mesh_data(const char *name, hid_t loc)
{
    ...
    h5io2_link_create(lname, "/VTKHDF/NumberOfPoints", "NumberOfPoints", loc);
    h5io2_link_create(lname, "/VTKHDF/NumberOfCells", "NumberOfCells", loc);
    h5io2_link_create(lname, "/VTKHDF/NumberOfConnectivityIds", "NumberOfConnectivityIds", loc);

    h5io2_link_create(lname, "/VTKHDF/Points", "Points", loc);
    h5io2_link_create(lname, "/VTKHDF/Offsets", "Offsets", loc);
    h5io2_link_create(lname, "/VTKHDF/Connectivity", "Connectivity", loc);
    h5io2_link_create(lname, "/VTKHDF/Types", "Types", loc);

    hid_t group = h5io2_group_create("CellData", loc);
    h5io2_link_create(lname, "/VTKHDF/CellData/vtkGhostType", "vtkGhostType", group);
    h5io2_link_create(lname, "/VTKHDF/CellData/entity", "entity", group);
    h5io2_link_create(lname, "/VTKHDF/CellData/rank", "rank", group);
    h5io2_link_create(lname, "/VTKHDF/CellData/volume", "volume", group);
    h5io2_link_create(lname, "/VTKHDF/CellData/center", "center", group);
...
static void write_field_data(const Equations *eqns, double time, hid_t loc)
{
    ...
    hid_t group = h5io2_group_create("FieldData", loc);
    h5io2_dataset_write("time", &time, root, 1, H5T_NATIVE_DOUBLE, group);
    for (int i = 0; i < num; i++) {
        h5io2_dataset_write(name[i], &property[i], root, 1, H5T_NATIVE_DOUBLE, group);
    }
    h5io2_group_close(group);
}
...
static void write_variables(const EquationsVariables *variables, int num_cells, hid_t loc)
{
    ...
    void *buf_ = teal2_calloc(num_cells, (int)sizeof(double[cap]));
